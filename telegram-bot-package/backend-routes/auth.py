...
def telegram_login(data: TelegramLoginRequest, db: Session = Depends(get_db)):
    """
    Instant login for users who registered via the Telegram bot.
    Accepts a Telegram chat_id and returns JWT tokens directly (no OTP).
    """
    user = db.query(User).filter(User.telegram_chat_id == data.chat_id).first()
    if not user:
        # Also check if a user was registered with the bot email pattern
        bot_email = f"tg_{data.chat_id}@invox.bot"
        user = db.query(User).filter(User.email == bot_email).first()
    ...
    if not user:
        raise HTTPException(
            status_code=404,
            detail="No account found for this Telegram chat ID. Please register via the Telegram bot first."
        )

    if not user.is_active:
        raise HTTPException(status_code=403, detail="Account is disabled")

    # Auto-verify and generate tokens
    user.is_verified = True

    # Auto-link vendor if not already linked
    if user.role == "vendor" and user.vendor_id is None:
        vendor = db.query(Vendor).filter(Vendor.email == user.email).first()
        if vendor:
            user.vendor_id = vendor.id
    ...
    log_activity(db, "user", user.id, "telegram_login", "Instant login via Telegram chat ID", user.id)
    db.commit()

    return {
        "message": "Telegram instant login successful",
        "auto_verified": True,
        "access_token": access_token,
        "refresh_token": refresh_token,
        "token_type": "bearer",
